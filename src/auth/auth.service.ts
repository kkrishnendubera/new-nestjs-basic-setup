...
export class AuthService {
    ...
    async generateRefreshToken(accessToken: string, user: string | Types.ObjectId): Promise<string> {
    ...
    async refreshToken(body: RefreshJwtDto): Promise<ApiResponse> {
        const refreshTokenHash = await hash(body.accessToken.split('.')[2] + body.refreshToken, body.refreshToken);

        const refreshTokenData = await this.refreshTokenRepository.getByField({ hash: refreshTokenHash });
        ...
        return { message: Messages.REFRESH_TOKEN_ISSUED_SUCCESS, data: { accessToken: token, refreshToken: salt } };
    }
    ...
    async userSignup(body: UserSignupDTO, files: Express.Multer.File[]): Promise<ApiResponse> {
        ...
        return { message: Messages.USER_REGISTRATION_SUCCESS, data: { user: userDetails, accessToken: token, refreshToken: refreshToken } };
    }
    ...
    async userLogin(body: UserSignInDTO, req: Request): Promise<ApiResponse> {
        ...
        return { message: Messages.USER_LOGIN_SUCCESS, data: { user: userDetails, accessToken: token, refreshToken: refreshToken } };
    }
